function [SIM,filter]=SIM_Wienerfilter3D_nt(s_class,obj,FFTSIM,qvector,Filter3D)
nz_RAW=obj.sizeZ;
if mod(nz_RAW,2)==0
    nz=nz_RAW+3;
else
...
if max(Filter3D(:))==0
    ...
    obj.kz=kz;
    ap_range = obj.pixelsize*obj.w*2*obj.NA/ ...
        obj.lambda+10;
    [filter] = Wienerfilter3D(OTFcombine,s_class,otfobj,obj.Wiener,obj.Zstep,notch,OTF_double,obj,qvector,ap_range);
else
    filter=Filter3D;
end
