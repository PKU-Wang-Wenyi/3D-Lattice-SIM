...
for i=1:length(param.kAmean)
    ...
    if k<pitch*0.75
        shift_OTF=shift_OTF+param.modFac(i)*spe_move(OTF_notch.^2,[round([param.kAmean(i,2),param.kAmean(i,1)]),kz]);
        shift_OTF=shift_OTF+param.modFac(i)*spe_move(OTF_notch.^2,[round([-param.kAmean(i,2),-param.kAmean(i,1)]),kz]);
        shift_OTF=shift_OTF+param.modFac(i)*spe_move(OTF_notch.^2,[round([param.kAmean(i,2),param.kAmean(i,1)]),-kz]);
        shift_OTF=shift_OTF+param.modFac(i)*spe_move(OTF_notch.^2,[round([-param.kAmean(i,2),-param.kAmean(i,1)]),-kz]);
    else
        shift_OTF=shift_OTF+param.modFac(i)*spe_move(OTF_notch.^2,round([param.kAmean(i,2),param.kAmean(i,1)]));
        shift_OTF=shift_OTF+param.modFac(i)*spe_move(OTF_notch.^2,round([-param.kAmean(i,2),-param.kAmean(i,1)]));
    end
end
