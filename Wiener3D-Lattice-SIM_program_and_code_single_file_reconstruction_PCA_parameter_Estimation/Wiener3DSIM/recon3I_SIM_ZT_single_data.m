function [SIM_all,WF_all]=recon3I_SIM_ZT_single_data(obj)
rawdata_all=single(tiffreadVolume(obj.imgname));
assert(size(rawdata_all,3)==13*obj.sizeZ*obj.sizeT)
rawdata_all=reshape(rawdata_all,[size(rawdata_all,1),size(rawdata_all,2),13*obj.sizeZ,obj.sizeT]);
for numT=1:obj.sizeT
    rawdata=rawdata_all(:,:,:,numT);
    %%
    ...
    if numT==1
        ...
        [s_class,coe]= KapproxEstimationF_PCA(s_class,obj);
        % [s_class.kAmean,coe] = kmeanF_PCA(s_class);
        % paramfiles=['saved_data_',num2str(obj.exlambda),'.mat'];
        % s_class.phaseA = phasesF_PCA(s_class);
        s_class = PCMseparateF_3D(s_class);
        % s_class.modFac = ModulationFactorF_PCA(s_class);
    ...
    [SIM_all,Filter3D]=SIM_Wienerfilter3D_nt(s_class,obj,s_class.fftDirectlyCombined,Kotf*coe,Filter3D);
    WF_all=WF;
    SIM_all=im2uint16(mat2gray(SIM_all(1:2*sizeX,1:2*sizeY,:)));
    WF_all=im2uint16(mat2gray(WF_all));
    filename=['_T',num2str(numT)];
    imwritestack_16(SIM_all,strrep(obj.resultname,'TEMP_STR',['SIM-Wiener',filename ]))
